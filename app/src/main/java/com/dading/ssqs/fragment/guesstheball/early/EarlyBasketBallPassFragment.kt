...
class EarlyBasketBallPassFragment : Fragment(), OnRefreshListener, NotificationController.NotificationControllerDelegate {
            ...
            SSQSApplication.apiClient(0).getMatchBasketBallResult(params, "0") { result ->
                if (result.isOk) {
                    ...
                    if (items?.isNotEmpty()) {
                        ...
                        ToastUtils.midToast(context, "篮球-综合过关 暂无数据!!", 0)
                    }
                } else {
    ...
    private fun initView(): View {
        ...
        tvTitle.text = "早盘-篮球:" + LocaleController.getString(R.string.scroll_title14)
        ...
        filterCell!!.setSelectClickListener {
            ...
            //判断是否有联赛的数据  没有的话网路请求
            if (DataController.getInstance().earlyBaskteBallData == null) {
                DataController.getInstance().syncEarlyBasketBall(TAG, currSelectTime)
                loadingDialog!!.show()
            } else {
                selectMatchDialog!!.show(DataController.getInstance().earlyBaskteBallData, DataController.getInstance().earlyBasketBallHotData, "联赛选择")
            }
        }
    ...
    private fun getNetDataWork(off: Int, lim: Int) {
        ...
        SSQSApplication.apiClient(0).getMatchBallGuessEarlyList(currSelectTime, sType, leagueIDs, off, lim) { result ->
            loadingDialog!!.dismiss()
            swipeToLoadLayout!!.isRefreshing = false
            swipeToLoadLayout!!.setRefreshEnabled(true)

            if (result.isOk) {
                val bean = result.data as EarlyBean

                filterCell!!.setCurrPage(off)

                if (bean?.leagueName != null && bean.leagueName.size >= 1) {
                    defaultView!!.visibility = View.GONE

                    totalPage = bean.totalCount
    ...
    companion object {
        private val TAG = "EarlyBasketBallPassFragment"
    }
}
