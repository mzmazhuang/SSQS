...
public class BResultFragment extends Fragment implements OnRefreshListener, OnLoadMoreListener, NotificationController.NotificationControllerDelegate {

    public static final String TAG = "BResultFragment";
    ...
    private void setSelectTime(int day) {
        ...
        NotificationController.getInstance().postNotification(NotificationController.scoreBasketChildPage, "2", currTime);

        ScoreDataController.Companion.getInstance().clearBasketBallResult();
    }
    ...
    private void getNetDataWork(String leagusId, int off, int lim, final boolean isRefre) {
        ...
        SSQSApplication.apiClient(0).getMatchBallOrTypeList(false, 3, currTime, "0", 0, leagusId, off, lim, new CcApiClient.OnCcListener() {
            @Override
            public void onResponse(CcApiResult result) {
