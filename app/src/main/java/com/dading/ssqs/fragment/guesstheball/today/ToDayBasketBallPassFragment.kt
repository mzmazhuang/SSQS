...
class ToDayBasketBallPassFragment : Fragment(), OnRefreshListener, NotificationController.NotificationControllerDelegate {
    ...
    private val readyListener = ScrollBallCell.OnReadyListener { title ->
        ...
        for (i in originalData!!.indices) {
            if (originalData!![i].leagueName == title) {
                networkData.add(originalData!![i])
            }
        }
    ...
    private fun initView(): View {
        ...
        filterCell!!.setSelectClickListener {
            ...
            //判断是否有联赛的数据  没有的话网路请求
            if (DataController.getInstance().todayBaskteBallData == null) {
                DataController.getInstance().syncBasketBall(TAG, 2)
                loadingDialog!!.show()
            } else {
                selectMatchDialog!!.show(DataController.getInstance().todayBaskteBallData, DataController.getInstance().todayBasketBallHotData, "联赛选择")
            }
        }
    ...
    private fun getNetDataWork(off: Int, lim: Int) {
        ...
        SSQSApplication.apiClient(0).getMatchBallOrTypeList(false, 2, mDate, "0", sType, leagueIDs, off, lim) { result ->
            loadingDialog!!.dismiss()
    ...
    companion object {
        private val TAG = "ToDayBasketBallPassFragment"
    }
}
