package com.dading.ssqs.components.tabindicator;
...
/*
 *  @项目名：  TabIndicator 
 *  @包名：    org.com.dading.ssqs.components.tabindicator.library
 *  @文件名:   TabIndicator
 *  @创建者:   Administrator
 */
public class TabIndicator extends HorizontalScrollView {
    ...
    private int     mTabTextColor     = R.color.tab_textcolor_selector;//选中标题颜色
    private float   mTabTextSize      = 15;
    ...
    private float[] getCurrentLeftAndRight() {
        ...
        if (currentTab != null) {
            mLeft = currentTab.getLeft();
            mRight = currentTab.getRight();
        }
        if (mPagerOffset > 0f && mCurrentPosition < mViewPager.getAdapter()
            ...
            //计算偏移后的left和right
            mLeft = nextLeft * mPagerOffset + (1 - mPagerOffset) * mLeft;
            mRight = nextRight * mPagerOffset + (1 - mPagerOffset) * mRight;
        }

        return new float[]{mLeft,
                mRight};
    }
