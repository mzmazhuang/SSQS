...
public class ShoppingAddressActivity extends BaseActivity implements View.OnClickListener {
    private static final String TAG = "ShoppingAddressActivity";
    @Bind(R.id.shopping_addr_people_name)
    EditText mShoppingAddrPeopleName;
    @Bind(R.id.shopping_addr_people_phone)
    EditText mShoppingAddrPeoplePhone;
    @Bind(R.id.shopping_addr_place_shengshixian)
    TextView mShoppingAddrPlaceShengshixian;
    @Bind(R.id.shopping_addr_place_xiangxi)
    EditText mShoppingAddrPlaceXiangxi;
    @Bind(R.id.shopping_addr_place_go_ly)
    RelativeLayout mShoppingAddrPlaceGoLy;
    ...
    protected void initView() {
        mView = View.inflate(this, R.layout.activity_shopping_address, null);

        mPopView = View.inflate(this, R.layout.pop_sheng_shi_xian, null);

        mPopSsxLy = (RelativeLayout) mPopView.findViewById(R.id.pop_ssx_ly);
        mPopSsxCancel = (TextView) mPopView.findViewById(R.id.pop_ssx_cancel);
        mPopSsxConfirm = (TextView) mPopView.findViewById(R.id.pop_ssx_confirm);
        mPopSsxMainWheelview = (WheelView) mPopView.findViewById(R.id.pop_ssx_main_wheelview);
        mPopSsxSubWheelview = (WheelView) mPopView.findViewById(R.id.pop_ssx_sub_wheelview);
        mPopSsxChildWheelview = (WheelView) mPopView.findViewById(R.id.pop_ssx_child_wheelview);
    ...
    protected void initData() {
        ...
        if (addMsg != null) {
            ...
            mIsHave = true;
            if (mBean!= null) {
                ShoppingAddBean entity = mBean;
                mShoppingAddrPeopleName.setText(entity.receiver);
                mShoppingAddrPeoplePhone.setText(entity.mobile);
                String s = entity.province + "-" + entity.city + "-" + entity.area;
                mShoppingAddrPlaceShengshixian.setText(s);
                mShoppingAddrPlaceXiangxi.setText(entity.address);
            }
        }

        String SSX = getString();
        SsxCity ssxCity = JSON.parseObject(SSX, SsxCity.class);
        List<SsxCity.CitylistEntity> citylist = ssxCity.citylist;
        ...
        HashMap<String, List<String>> mapSub = new HashMap<>();
        HashMap<String, List<String>> mapChild = new HashMap<>();
        ...
        for (SsxCity.CitylistEntity ssx : citylist) {
             ...
             * 如果是直辖市name就没有县
             */
            List<SsxCity.CitylistEntity.CEntity> c = ssx.c;

            for (SsxCity.CitylistEntity.CEntity city : c) {
                listX = new ArrayList<>();
                String n = city.n;//得到市名
                listC.add(n);//市集合map
                if (ssx.p.equals("北京") || ssx.p.equals("天津") || ssx.p.equals("上海") || ssx.p.equals("重庆")
                        || ssx.p.equals("香港") || ssx.p.equals("澳门") || ssx.p.equals("台湾")) {
                    //直辖市内没有县对象集合 : a
                    for (int i = 0; i < 1; i++) {
                        listX.add("--");
                    }
                    ...
                    //得到每个市区内县的集合
                    List<SsxCity.CitylistEntity.XEntity> a = city.a;
                    for (SsxCity.CitylistEntity.XEntity s : a) {
                        listX.add(s.s);//县集合
                    }
                }
                mapChild.put(n, listX);//市-县集合关联
            }
            mapSub.put(ssx.p, listC);//省-市集合关联

        }

        mPopSsxMainWheelview.setWheelAdapter(new ArrayWheelAdapter(this));
        mPopSsxMainWheelview.setSkin(WheelView.Skin.Holo);
        //省集合
        mPopSsxMainWheelview.setWheelData(listP);
        WheelView.WheelViewStyle style = new WheelView.WheelViewStyle();

        style.selectedTextSize = 16;
        style.textSize = 13;
        style.backgroundColor = getResources().getColor(R.color.gray_d);
        mPopSsxMainWheelview.setStyle(style);
        mPopSsxSubWheelview.setWheelAdapter(new ArrayWheelAdapter(this));
        mPopSsxSubWheelview.setSkin(WheelView.Skin.Holo);

        //市的集合
        List<String> list = mapSub.get(listP.get(mPopSsxMainWheelview.getSelection()));
        mPopSsxSubWheelview.setWheelData(list);

        mPopSsxSubWheelview.setStyle(style);
        mPopSsxMainWheelview.join(mPopSsxSubWheelview);
        mPopSsxMainWheelview.joinDatas(mapSub);


        mPopSsxChildWheelview.setWheelAdapter(new ArrayWheelAdapter(this));
        mPopSsxChildWheelview.setSkin(WheelView.Skin.Holo);

        mPopSsxChildWheelview.setWheelData(mapChild.get(mapSub.get(listP.get(mPopSsxMainWheelview
                .getSelection())).get(mPopSsxSubWheelview.getSelection())));
        mPopSsxChildWheelview.setStyle(style);
        mPopSsxSubWheelview.join(mPopSsxChildWheelview);
        mPopSsxSubWheelview.joinDatas(mapChild);
    }
    ...
    protected void initListener() {
        mPopu.setOnDismissListener(new PopupWindow.OnDismissListener() {
            ...
            public void onDismiss() {
                mShoppingAddrPlaceGoLy.setClickable(true);
            }
        });
        mPopSsxLy.setOnClickListener(this);
        mPopSsxCancel.setOnClickListener(this);
        mPopSsxConfirm.setOnClickListener(this);
    }

    @Override
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.pop_ssx_ly:
            case R.id.pop_ssx_cancel:
                mPopu.dismiss();
                break;
            case R.id.pop_ssx_confirm:
                StringBuilder sb = new StringBuilder();
                mSheng = mPopSsxMainWheelview.getSelectionItem();
                mShi = mPopSsxSubWheelview.getSelectionItem();
                mXian = mPopSsxChildWheelview.getSelectionItem();
                if (mXian.equals("--")) {
                    sb.append(mSheng.toString()).append(mShi.toString());
                } else {
    ...
    @OnClick({R.id.shopping_addr_back, R.id.shopping_addr_save, R.id.shopping_addr_place_go_ly})
    public void OnClik(View v) {
        switch (v.getId()) {
            case R.id.shopping_addr_back:
                finish();
                break;
            case R.id.shopping_addr_save:
                String name = mShoppingAddrPeopleName.getText().toString();
                String phone = mShoppingAddrPeoplePhone.getText().toString();
                String sSxPlace = mShoppingAddrPlaceShengshixian.getText().toString();
                ...
                if (sSxPlace.equals("北京") || sSxPlace.equals("天津") || sSxPlace.equals("上海") || sSxPlace.equals("重庆")
                        || sSxPlace.equals("香港") || sSxPlace.equals("澳门") || sSxPlace.equals("台湾")) {
                    switch (sSxPlace.substring(0, 3)) {
                        case "北京":
                            mAddr = "北京";
                            break;
                        case "天津":
                    ...
                    if (mIsHave) {
                        mAddr = mBean.province;
                    } else {
                        mAddr = mSheng.toString();
                    }
                }
                ...
                if (mIsHave) {
                    ...
                    mArea = mXian.toString();
                    mCity = mShi.toString();
                }

                AddressInfoElement element = new AddressInfoElement();
                element.setAddress(xxPlace);
                element.setMobile(phone);
                element.setProvince(mAddr);
                element.setCity(mCity);
                element.setArea(mArea);
                element.setReceiver(name);

                SSQSApplication.apiClient(classGuid).getAddressInfo(element, new CcApiClient.OnCcListener() {
                    @Override
                    public void onResponse(CcApiResult result) {
                        if (result.isOk()) {
                            ...
                            if (403 == result.getErrno()) {
                                ...
                                Intent intent = new Intent(ShoppingAddressActivity.this, LoginActivity.class);
                                startActivity(intent);
                                finish();
                            } else {
                ...
                break;
            case R.id.shopping_addr_place_go_ly:
                //选择省市县
                mPopu.showAtLocation(mView, Gravity.BOTTOM, 0, 0);
                mShoppingAddrPlaceGoLy.setClickable(false);
                mIsHave = false;
                break;
            default:
                break;
