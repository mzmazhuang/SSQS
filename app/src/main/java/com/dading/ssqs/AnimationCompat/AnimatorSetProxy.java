...
import com.dading.ssqs.Animation.AnimatorSet10;
...
public class AnimatorSetProxy{
    ...
    private final Object animatorSet;
    ...
    private static <T, U> T[] copyOfRange(U[] original, int start, int end, Class<? extends T[]> newType) {
        if (start > end) {
    ...
    public void playSequentially(ArrayList<Object> items){
        if (View10.NEED_PROXY) {
            ArrayList<Animator10> animators = new ArrayList<>();
            for (Object obj : items) {
                animators.add((Animator10)obj);
            }
            ((AnimatorSet10) animatorSet).playSequentially(animators);
        } else {
            ArrayList<Animator> animators = new ArrayList<>();
            for (Object obj : items) {
                animators.add((Animator)obj);
            }
            ((AnimatorSet) animatorSet).playSequentially(animators);
        }
    }
