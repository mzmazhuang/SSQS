...
class ScrollBallDefaultFragment : Fragment(), OnRefreshListener, NotificationController.NotificationControllerDelegate {

    private var swipeToLoadLayout: SwipeToLoadLayout? = null
    private var mRecyclerView: RecyclerView? = null
    private var adapter: ScrollBallDefaultAdapter? = null
    ...
    private var sType = 0//0==时间排序 1==联赛排序
    ...
    private fun initView(): View {
        ...
        adapter = ScrollBallDefaultAdapter(context)
        ...
        SSQSApplication.apiClient(0).getScrollBallList(true, 6, mDate, sType, leagueIDs, off, lim) { result ->
            ...
            if (result.isOk) {
                ...
                if (page?.items != null && page.items.size >= 1) {
                    ...
                    adapter!!.data = getData(getFilterData(page.items))
                } else {
                    adapter!!.clearData()
    ...
    class MergeBean : Serializable {
        ...
        var bean: MutableList<ScrollBallFootBallBean.ScrollBeanItems.ScrollBeanItem>? = null
        var items: ScrollBallFootBallBean.ScrollBeanItems? = null
    ...
    companion object {
        private val TAG = "ScrollBallDefaultFragment"
    }
}