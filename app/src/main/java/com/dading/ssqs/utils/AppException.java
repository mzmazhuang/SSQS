...
public class AppException implements UncaughtExceptionHandler {
    ...
    private boolean handleException(Throwable ex) {
        ...
        new Thread() {
            ...
            public void run() {
                Looper.prepare();
                //Toast.makeText(mContext, "很抱歉,程序崩溃了,即将退出.", Toast.LENGTH_LONG).show();
                Looper.loop();
            }
        }.start();
        //收集设备参数信息
    ...
    private String saveCrashInfo2File(Throwable ex) {
        ...
        try {
            ...
            if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
                ...
                fos.close();
                Log.d(TAG, "返回数据是------------------------------:" + dir.getAbsolutePath());
                postAsynFile(path + "/" + mFileName);
            }
            //todo 上传至服务器待写
            return mFileName;
        } catch (Exception e) {
    ...
    private void postAsynFile(String dir) {
        ...
        RequestBody requestBody = new MultipartBody.Builder()
                ...
                .addFormDataPart("txt", mFileName, fileBody)
                .build();
        Request request = new Request.Builder()
                .url(SSQSApplication.apiClient(0).getBaseUri() + "/v1.0/errMsg")
                .post(requestBody)
                .build();
