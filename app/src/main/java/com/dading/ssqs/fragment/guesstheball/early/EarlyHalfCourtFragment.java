...
public class EarlyHalfCourtFragment extends Fragment implements OnRefreshListener, NotificationController.NotificationControllerDelegate {

    private static final String TAG = "EarlyHalfCourtFragment";
    ...
    private List<ScrollBallHalfCourtFragment.MergeBean> leagusList = new ArrayList<>();
    ...
    private View initView() {
        ...
        tvTitle.setText("早盘-足球:" + LocaleController.getString(R.string.scroll_title4));
            ...
            if (leagueMatchId == id || leagueMatchId == -1) {
                ...
                if (isAdd) {//是添加 还是删除
                    if (leagusList.size() == 0) {
                        ScrollBallHalfCourtFragment.MergeBean mergeBean = new ScrollBallHalfCourtFragment.MergeBean();
                        ...
                        for (int i = 0; i < leagusList.size(); i++) {
                            ScrollBallHalfCourtFragment.MergeBean mergeBean = leagusList.get(i);
                    ...
                    Iterator<ScrollBallHalfCourtFragment.MergeBean> mergeBeanIterator = leagusList.iterator();

                    while (mergeBeanIterator.hasNext()) {
                        ScrollBallHalfCourtFragment.MergeBean mergeBean = mergeBeanIterator.next();
    ...
    private ScrollBallCommitMenuAdapter.OnMenuClickListener menuListener = new ScrollBallCommitMenuAdapter.OnMenuClickListener() {
        ...
        public void onClick(int position, final int dataId, final int itemId, final String value) {
            ...
            Thread thread = new Thread(new Runnable() {
                @Override
                public void run() {
                    Iterator<ScrollBallHalfCourtFragment.MergeBean> iterator = leagusList.iterator();

                    while (iterator.hasNext()) {
                        ScrollBallHalfCourtFragment.MergeBean mergeBean = iterator.next();
    ...
    private List<ScrollBallFootBallHalfCourtBean.ScrollBallFootBallHalfCourtItems> handleOtherData(List<JCScorebean> items, List<EarlyBean.EarlyMatchs> currData) {
        ...
        for (int i = 0; i < currData.size(); i++) {
            ...
            data.add(new ScrollBallFootBallHalfCourtBean.ScrollBallFootBallHalfCourtItem(currData.get(i).getHome() + currData.get(i).getAway()));
