...
public class FScheduleFragment extends Fragment implements OnRefreshListener, OnLoadMoreListener, NotificationController.NotificationControllerDelegate {

    public static final String TAG = "FScheduleFragment";
    ...
    private void setSelectTime(int day) {
        ...
        NotificationController.getInstance().postNotification(NotificationController.scoreFootChildPage, "3", currTime);

        ScoreDataController.Companion.getInstance().clearFootBallSchedule();
    }
    ...
    private void getNetDataWork(String leagusId, int off, int lim, final boolean isRefre) {
        ...
        SSQSApplication.apiClient(0).getMatchBallOrTypeList(true, 4, currTime, "0", 0, leagusId, off, lim, new CcApiClient.OnCcListener() {
            @Override
            public void onResponse(CcApiResult result) {
